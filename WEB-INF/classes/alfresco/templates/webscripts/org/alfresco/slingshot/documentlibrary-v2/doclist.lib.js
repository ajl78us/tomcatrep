...
//***************************************************** MIK change 
	...
	if(filterParams.isMikReadOnlyUser){
		//get total record
...
function doclist_main()
{
   ...
   if (logger.isLoggingEnabled())
      logger.log("doclist.lib.js - NodeRef: " + parsedArgs.nodeRef + " Query: " + query + " isMikReadOnlyUser: " +filterParams.isMikReadOnlyUser);
            ...
            if (!allSites || node.qnamePath.match(pathRegex))
            {
               ...
               if (node.isContainer || node.isLinkToContainer)
               {
            	   if(!people.isAdmin(person)){
            		   if(!(node.properties.name == "Data Dictionary")){
