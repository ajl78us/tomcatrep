 ...
 */
function getQueryTemplate(siteId)
{
...
var name = encodeURIComponent(person.properties.userName);	
	...
	if(siteId != null){
		site =siteId;
	//	logger.log("Live Search Site ID: "+site);
	}
	if(site != null && site != ''){
		siteUrl = siteService.getSite(site);
	//	logger.log("Live Search Site Url: "+siteUrl);
	}
	...
	var templateStr =  "%(cm:name cm:title cm:description TEXT TAG)  AND (ISNULL:'miklink:persona' AND ISNULL:'miklink:url' AND ISNULL:'miklink:startDate') AND (mikdoc:isArchived:false)"; // miklink null to restrict links in live search
	...
	if(isSupportCenter){
		...
		templateStr  = templateStr  + "  AND mikdoc:persona:'Team Member'"; //default persona
	}	
	templateStr = templateStr + " OR ( %(cm:name cm:title cm:description) +TYPE:\"cm:folder\")";
	}
...
function getDocResults(params)
{
   ...
   var queryDef = {
      ...
      templates: getQueryTemplate(params.siteId),
   ...
   for (var i=0, item; i<nodes.length && i<params.maxResults; i++)
   {
      ...
      catch (e)
      {
         ...
         {
        	 logger.warn("Live Search Node Ref:: " + nodes[i].nodeRef + " ::Skipping this node due to exception when processing query result: " + e);
         }
      }
   }
